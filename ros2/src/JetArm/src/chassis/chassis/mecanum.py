...
class MecanumChassis:
    ...
    def __init__(self, a=103, b=97, wheel_diameter=96.5, pulse_per_cycle=44 * 178):
        self.motor_controller = motor.EncoderMotorController(7)
        self.a = a
        self.b = b
            ...
            if velocity_y == 0:
                ...
                if velocity_x < 0:
                    ...
                    if velocity_y < 0:
                        direction += 360
        if fake:
            return velocity, float(direction)
        else:
            return self.set_velocity(velocity, float(direction), 0)
...
def main() :      
    MEC=MecanumChassis()
    MEC.translation(30, 30 , fake=False)
    time.sleep(2)
    MEC.translation(0, 0 , fake=False)
