...
class YoLov8TRT(object):
    ...
    def infer(self, raw_image_generator):
        ...
        for i in range(self.batch_size):
            #print("output[i * self.det_output_length: (i + 1) * self.det_output_length]", output[i * self.det_output_length: (i + 1) * self.det_output_length])
            #print("batch_origin_h[i]", batch_origin_h[i])
            #print("batch_origin_w[i]", batch_origin_w[i])
            result_boxes, result_scores, result_classid = self.post_process(output[i * self.det_output_length: (i + 1) * self.det_output_length], batch_origin_h[i],batch_origin_w[i],rotated=True)
    ...
    def post_process(self, output, origin_h, origin_w, rotated=False):
        """
        ...
        boxes = self.non_max_suppression(pred, origin_h, origin_w,
                                         conf_thres=CONF_THRESH, nms_thres=IOU_THRESHOLD, rotated=True)
        if rotated:
    ...
    def non_max_suppression(self, prediction, origin_h, origin_w, conf_thres=0.5, nms_thres=0.4, rotated=False):
        """
        ...
        col_idx = []
        if rotated:
            # obb_bbox [center_x, center_y, w, h, conf, cls_id, angle]
            col_idx = [0, 1, 2, 3, 89]
        else:
...
if __name__ == "__main__":
    ...
    running = True
    if test:
        PLUGIN_LIBRARY = "/home/hiwonder/jetarm_ros2_ws/src/JetArm/src/example/example/yolov8/libmyplugins.so"
        engine_file_path = "/home/hiwonder/jetarm_ros2_ws/src/JetArm/src/example/example/yolov8/garbage_classification_640s.engine"
